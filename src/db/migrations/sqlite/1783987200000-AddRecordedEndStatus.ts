...
export class AddRecordedEndStatus1783987200000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE "recorded" ADD COLUMN "endStatus" smallint NOT NULL DEFAULT (0)`);
        // failReason は将来の失敗理由記録 (tuner不足判定等) 用の休眠 column (migration 再発行の回避)
        await queryRunner.query(`ALTER TABLE "recorded" ADD COLUMN "failReason" smallint NOT NULL DEFAULT (0)`);
    }
